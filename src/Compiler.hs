...
-- section 2.3
compile (DL.Fun v args Nothing) = do
...
compile (DL.Fun v args (Just cv)) = do
...
compile (DL.Case t pts) = do
    ...
    let rhss = compile . (\(DL.MkClause _ _ t1) -> t1) <$> pts
    let cv = freshCovar (t' : rhss)
    let rhss' = (\p -> Core.Cut p (Core.Covar cv)) <$> rhss
    let ptFun (DL.MkClause ct v _, c) = Core.MkPattern ct v [] c
    let pts' = ptFun <$> zip pts rhss'
    Core.Mu cv (Core.Cut t' (Core.Case pts'))
compile (DL.Cocase patterns) = do
    -- to compile cocases we only need to compile all patterns 
    Core.Cocase (compilePattern <$> patterns)
  where
    compilePattern :: DL.Clause DL.Dtor -> Core.Pattern DL.Dtor
    compilePattern (DL.MkClause dt vars t) = do
...
compile (DL.App t1 t2) = do
    let (t1', t2') = (compile t1, compile t2)
    let cv = freshCovar [t1', t2']
    let newD = Core.Destructor DL.Ap [t2'] [Core.Covar cv]
...
compile (DL.Label cv t) = do
    Core.Mu cv (Core.Cut (compile t) (Core.Covar cv))

-- section 2.3
-- compile a definition
-- works regardless of if its typed or not 
compileDef :: forall a. DL.Def a -> Core.Def a
compileDef (DL.Def nm prodargs Nothing bd rt) = do
...
compileDef (DL.Def nm prodargs (Just cv) bd rt) = do
...
compileProg :: forall a. DL.Prog a -> Core.Prog a
compileProg (DL.MkProg defs) = Core.MkProg (compileDef <$> defs)
